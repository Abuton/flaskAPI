        ...
        if request.method == "POST":
            ...
            if customer is not None:
                ...
                if query.cust_ssn_id is None:
                    return jsonify(success=False, status_code=403, message="Data is not inserted! Check you input")
                else:
                    temp = CustomerLog(
        ...
        if cust_ssn_id is not None:
            if request.method != "POST":
                ...
                if data is not None and data.status != "deactivate":
                    ...
                    return jsonify(success=False, status_code=404, message="Customer Info not found")
            else:
                cust_ssn_id = int(cust_ssn_id)
                name = request.form.get("name")
                ...
                if customer_info is not None:
                    ...
                    db.add(temp)
                    db.commit()
                    return jsonify(success=False, status_code=200, message="Customer data are updated successfully")
                else:
                    return jsonify(success=False, status_code=403, message="Invalid customer Id. Please, check customer Id.")
...
def deletecustomer()):
    ...
    if session["usert"] == "employee":
        if cust_ssn_id is not None:
            ...
            return jsonify(sucess=False, status_code=403, message="Customer ID can't be None")
        ...
        if request.method == "POST":
            ...
            if customer_info is not None:
                ...
                if query.acc_id is None:
                    jsonify(success=False, status_code=403,
...
def transfer():
    ...
    if session["usert"] == "employee":
        if cust_ssn_id is None:
            return jsonify(success=False, status_code=404,
                            message="Customer ID can't be None")
        else:
            if request.method == "POST":
                # get data from form
        ...
        if request.method == "GET":
            ...
            if account is not None and len(account) >= 1:
                ...
                if transfer_history is not None:
                    transfer_history = jsonify(success=True, status_code=200,
